/*
 *  Copyright 2016 The DropRTC Project Authors. All rights reserved.
 *
...
// Surfaces AVAudioSession events. DropRTC will listen directly for notifications
...
/** Called on a DropRTC thread when the audio device is notified to begin
 *  playback or recording.
 */
- (void)audioSessionDidStartPlayOrRecord:(RTC_OBJC_TYPE(RTCAudioSession) *)session;

/** Called on a DropRTC thread when the audio device is notified to stop
 *  playback or recording.
 */
 ...
 *  DropRTC and the application layer are avoided.
 *
...
/** If YES, DropRTC will not initialize the audio unit automatically when an
 *  audio track is ready for playout or recording. Instead, applications should
 *  call setIsAudioEnabled. If NO, DropRTC will initialize the audio unit
 *  as soon as an audio track is ready for playout or recording.
 */
@property(nonatomic, assign) BOOL useManualAudio;

/** This property is only effective if useManualAudio is YES.
 *  Represents permission for DropRTC to initialize the VoIP audio unit.
 *  When set to NO, if the VoIP audio unit used by DropRTC is active, it will be
 *  stopped and uninitialized. This will stop incoming and outgoing audio.
 *  When set to YES, DropRTC will initialize and start the audio unit when it is
