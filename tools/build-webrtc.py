...
def build(target_dir, platform, debug):
    ...
    if platform == 'ios':
        # XCFramework
        xcodebuild_cmd = 'xcodebuild -create-xcframework -output %s' % os.path.join(build_dir, 'DropRTC.xcframework')
        for arch in IOS_BUILD_ARCHS:
            gn_out_dir = 'out/%s-%s' % (build_type, arch)
            xcodebuild_cmd += ' -framework %s' % os.path.join(gn_out_dir, 'DropRTC.framework')
        sh(xcodebuild_cmd)

        # XCFramework (stripped)
        xcodebuild_cmd = 'xcodebuild -create-xcframework -output %s' % os.path.join(build_dir, 'stripped', 'DropRTC.xcframework')
        bitcode_strip_cmd = 'xcrun bitcode_strip -r %s -o %s'
        for arch in IOS_BUILD_ARCHS:
            gn_out_dir = 'out/%s-%s' % (build_type, arch)
            framework_path = os.path.join(gn_out_dir, 'DropRTC.framework', 'DropRTC')
            sh(bitcode_strip_cmd % (framework_path, framework_path))
            xcodebuild_cmd += ' -framework %s' % os.path.join(gn_out_dir, 'DropRTC.framework')
        sh(xcodebuild_cmd)

        # dSYMs
        dsyms_dir = os.path.join(build_dir, 'DropRTC.dSYMs')
        for arch in IOS_BUILD_ARCHS:
            gn_out_dir = 'out/%s-%s' % (build_type, arch)
            shutil.copytree(os.path.join(gn_out_dir, 'DropRTC.dSYM'), os.path.join(dsyms_dir, 'DropRTC.framework.%s.dSYM' % IOS_ARCH_MAP[arch]))
    else:
